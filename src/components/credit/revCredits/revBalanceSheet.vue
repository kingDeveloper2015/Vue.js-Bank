...
<script>
	...
	import Modal from './create_modal_CU/balance_sheet.vue'
	...
	export default {
	    ...
	    computed: {
	    	rev_credits: function() {
				var temp = [];
				if ( !this.$root.$data.all_data.Revenue_balance_sheet) return;
				var sCreditID = this.$root.$data.sCreditID;
				for ( var i = 0 ;i< this.$root.$data.all_data.Revenue_balance_sheet.length; i++){
					var CID = this.$root.$data.all_data.Revenue_balance_sheet[i].CreditID;
					if ( CID == sCreditID ) {
						temp.push(this.$root.$data.all_data.Revenue_balance_sheet[i]);
					}
				}
				return temp;
	    	}
	    ...
	    methods: {
	        ...
	        click_delete: function() {
		        if (this.selected_item != -1) {
		        	//this.$root.$data.rev_financial_data['balance_sheet'].splice(this.selected_item, 1);
		        	this.server_request();
		        }
	        },
	        ...
	        server_request: function() {
				...
				lambda.invoke(pullParams, function(error,data) {
					if ( error) {
						...
						$this.$root.$data.all_data.Revenue_balance_sheet.splice($this.selected_item,1);
