...
<script>
	import Modal from './credit_modal_CU/gen_fund.vue'
	...
	export default{
        ...
        computed: {
	    	go_credits: function() {
				var temp = [];
				if ( !this.$root.$data.all_data.GO_general_fund) return;
				var sCreditID = this.$root.$data.sCreditID;
				for ( var i = 0 ;i< this.$root.$data.all_data.GO_general_fund.length; i++){
					var CID = this.$root.$data.all_data.GO_general_fund[i].CreditID;
					if ( CID == sCreditID ) {
						temp.push(this.$root.$data.all_data.GO_general_fund[i]);
					}
				}
				return temp;
	    	}
        ...
        methods: {
	        ...
	        click_delete: function() {
		        ...
		        if (this.selected_item != -1) {
		        	//this.$root.$data.go_financial_data['general_fund'].splice(this.selected_item, 1);
		        	this.server_request();
		        }
            ...
            server_request: function() {
				...
				lambda.invoke(pullParams, function(error,data) {
					if ( error) {
						...
						$this.$root.$data.all_data.GO_general_fund.splice($this.selected_item,1);
